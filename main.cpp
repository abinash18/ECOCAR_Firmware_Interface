...
#define BAUD_RATE 9600
	...
	int main(int argc, char **argv)
	{
		...
		check(sp_add_port_events(event_set_RX, ports[0], SP_EVENT_RX_READY));
		check(sp_add_port_events(event_set_TX, ports[0], SP_EVENT_TX_READY));
		// }
		printf("Waiting up to 5 seconds for RX on any port...\n");
		check(sp_wait(event_set_RX, 2000));

		// struct device p = {ports[0], event_set_RX};
		struct device *p;
		p->port = ports[0];
		p->event_set = event_set_RX;
		char *buffer;
		// char *buffer = (char *)calloc(32 + 1, sizeof(char));
		read_until(p, '\n', &buffer);
		...
		int result;
		check(sp_wait(event_set_TX, 1000));
		//  /* Send data. */
		...
		check(sp_wait(event_set_RX, 1000));
		...
		// printf("\x1b[31mRecieved string:\033[0m %s\n", buffer);
		// free(buffer);
		sp_free_event_set(event_set_RX);
		sp_free_event_set(event_set_TX);
		// for (int i = 0; i < num_ports; i++) {
...
// 	/* Send data. */
// 	// printf("Sending '%s' (%d bytes) on port %s.\n", data, size, sp_get_port_name(ports[0]));
// 	// result = check(sp_blocking_write(ports[0], data, size, 2000));
...
// 	struct device d = {ports[0], event_set};

// 	si_write(&d, &data, size);
